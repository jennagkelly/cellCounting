function applyManCorrectionsb(wkspctofix,chnls)
...
for boxnum=1:length(overlappix)-1
    for chnl=1:length(chnls)
        filename=strcat('CellCounter_auto_',filenameintro,num2str(boxnum),'b_',chnls{chnl},'.xml');
        markers=imageJCellCounterDataToMLBStruct(directory,filename,1);
        for marker=1:size(markers,1)
            zmatch=colocMat.(['box' num2str(boxnum)])(round(colocMat.(['box' num2str(boxnum)])(:,4))==rangesb.(['box' num2str(boxnum)])(1)+markers(marker,3)-1,:);

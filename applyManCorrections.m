function applyManCorrections(wkspctofix,chnls)
...
%         filename=strcat('CellCounter_auto_',filenameintro,num2str(boxnum),'_',chnls{chnl},'.xml');
        filename=strcat('CellCounter_auto',filenameintro,num2str(boxnum),'_',chnls{chnl},'.xml');
        markers=imageJCellCounterDataToMLBStruct(directory,filename,1);
        for marker=1:size(markers,1)
            zmatch=colocMat.(['box' num2str(boxnum)])(round(colocMat.(['box' num2str(boxnum)])(:,4))==ranges.(['box' num2str(boxnum)])(1)+markers(marker,3)-1,:);
